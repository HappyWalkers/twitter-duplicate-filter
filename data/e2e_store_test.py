...
EXT = REPO / "dist-store"
PROFILE = Path.home() / ".cache" / "x-dedup-store-e2e"
...
async def main():
    ...
    # For the store build this is an ASSERTION, not a diagnostic. The model is bundled
    # and allowRemoteModels is off, so any huggingface.co request means the packaged
    # files were not found and it silently fell back to the network -- which would fail
    # for real users (no host permission) and would contradict the store listing.
    hf = results.get("hfRequests", 0)
    print("OFFLINE       :", "PASS (0 network requests to huggingface.co)" if hf == 0
          else f"FAIL ({hf} HF requests -- bundled model not being used: {results.get('hfSample')})")
